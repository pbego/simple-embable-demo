...
| Problem | Fix |
|---------|-----|
| Embedding model not found at startup | `ollama pull nomic-embed-text`; set `embabel.models.default-embedding-model` to match `ollama list` exactly (e.g. `nomic-embed-text:latest`) |
...
| `Lock held by another program` on `write.lock` | Another JVM holds the index — `lsof ~/.simple-demo/lucene-index/write.lock` then `kill <pid>`; only one `spring-boot:run` at a time |
